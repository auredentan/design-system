...
(theme) => {
    ...
    const [activeChip, setActiveChip] = React.useState(INITIAL_STATE_ACTIVE_CHIP);
    const [filterValue, setFilterValue] = React.useState('');
    ...
    const onChange = (value) => {
        ...
        setShowSuggestions(value.length > 0);
        setNavigationSuggestionValue(INITIAL_STATE_NAVIGATION_SUGGESTION);
        setActiveChip(INITIAL_STATE_ACTIVE_CHIP);
    ...
    const onBackspacePressed = () => {
        if (isTextFieldEmpty() && selectedValues.length > 0) {
            ...
            setWasBackspacePressed(!wasBackspacePressed);
